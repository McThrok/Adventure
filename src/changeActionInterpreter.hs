...
evalChange ["flags"] change (StringValue value) = modify (\s -> s {gameFlags = (evalFlags change value (gameFlags s))})
...
evalObject ["flags"] change value obj = obj{objectFlags = evalFlags change value (objectFlags obj)}
...
evalLocation ["flags"] change (StringValue value) loc = loc{locationFlags = evalFlags change value (locationFlags loc)}
